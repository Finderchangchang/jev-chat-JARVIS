...
- 判断接口新增内置预设「博查 Jev」，排在选项第二（OpenRouter / 博查 Jev / TypeSafe 直连 / Vercel / OpenCode Zen / Opper / 自定义），选中后自动填好服务地址 `https://jev.bocha.cn` 与模型 `bocha-jev-v1`（协议与 TypeSafe 一致），页面上会显示官方地址并支持一键复制，当前限时免费。全新安装默认使用 OpenRouter；已经配置过判断接口的老用户不受影响，provider 和密钥都不会被改动。
...
- 判断接口另有「Opper」预设：地址 `https://api.opper.ai/v3/compat`，模型 `typesafe/jev-latest`（输出免费、输入 $0.042/M），密钥用 [Opper](https://opper.ai) 的 key。协议与 TypeSafe 直连相同（`POST /v1/systemone`）；同一地址也可把模型改成开源的 `opper/kev-4b`（[Kev 4B](https://huggingface.co/jaredpalmer/kev-4b)）。
- 内置 OpenRouter、博查 Jev、TypeSafe 直连、Vercel、OpenCode Zen、Opper、DeepSeek 官方、通义兼容预设，每张卡一键连通测试。
