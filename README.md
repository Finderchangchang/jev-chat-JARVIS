...
除了微信的无障碍读屏，还支持把**飞书**作为第二消息源：走飞书开放平台官方 API（机器人所在群的
消息只读轮询），分析内核与悬浮窗完全复用。客户端**只实现读接口**——没有能力代替你发出任何消息，
回复仅提供「复制」，粘贴发送由你手动完成。配置方法、权限清单与已知限制见
[docs/feishu.md](docs/feishu.md)；PC 端连通性自测脚本见 `tools/feishu/smoke_test.py`。
...
- `app/` — Android 应用（Kotlin，传统 View，无 Compose）
  - `capture/` 无障碍采集与前台保活 · `jev/` Jev 客户端与题目集 · `overlay/` 悬浮窗 · `core/` 配置与数据模型 · `feishu/` 飞书开放平台消息源（只读）
- `tools/jev/` — Jev 题目集与校准脚手架（Python，PC 上跑）
- `tools/feishu/` — 飞书连通性冒烟脚本（Python，PC 上跑）
- `docs/` — 设计与验收文档
